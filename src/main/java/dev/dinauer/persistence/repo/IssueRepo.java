...
import dev.dinauer.persistence.VersionedRepository;
import jakarta.enterprise.context.ApplicationScoped;
import jakarta.inject.Inject;
import dev.dinauer.similarity.SimilarityCalculator;
...
public class IssueRepo implements VersionedRepository<String, Issue>
{
    ...
    private void connectToSimilarIssues(RemoteDatabase remoteDB, Vertex from, Issue vertex)
    {
        ...
        // Filter only issues for current project
        List<VersionedEntity<String, Issue>> versionedEntities = findAllVersions().stream().filter(item -> item.getLatest().getProjectKey().equals(vertex.getProjectKey())).toList();
        for (VersionedEntity<String, Issue> version : versionedEntities)
        {
            if (version.getLatest() != null)
            {
                ...
                if (similarityOptional.isPresent() && similarityOptional.get() > 0.1)
                {
